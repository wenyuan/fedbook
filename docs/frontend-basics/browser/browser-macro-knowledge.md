...
下面总结一下这个问题该如何回答（不考虑渲染流程，如果要增加渲染流程的话，就是追加在这个过程的末尾）：
...
* 网络进程解析响应流程；
  * 检查状态码，如果是 301/302，则需要重定向，从 Location 自动中读取地址，重新进行第 4 步，如果是200，则继续处理请求。
...
* 渲染进程对文档进行页面解析和子资源加载，生成最终页面
  * 构建 DOM 树：通过 HTML 解析器将 HTML 转换成浏览器能够理解的 DOM 树。
  * 样式计算：渲染引擎将 CSS 文本转换为浏览器可以理解的结构 —— styleSheets，并对其进行属性值的标准化操作。然后计算出 DOM 树中每个节点的具体样式。
  * 布局：有了 DOM 树和 DOM 树中元素的样式，接下来创建布局树（只包含可见元素的树）并进行布局计算，最后将 DOM 元素的布局信息都保存在布局树中。

::: tip 从浏览器地址框输入 google.com 后会发生什么
